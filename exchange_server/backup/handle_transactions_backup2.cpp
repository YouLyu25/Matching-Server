...
int update_record (connection* C, int status,
                   std::string& sym, std::string& order_id,
  ...
  try {
    ...
    seller_new_shares_ld = seller_curr_shares_ld - amount_ld;
    buyer_new_balance_ld = buyer_curr_balance_ld - balance_diff_ld;
    buyer_new_shares_ld = buyer_curr_shares_ld + amount_ld;
    ...
    /*   3. TODO update record of finished orders (CLOSED_ORDER)   */
    if (status == SELL) {
    ...
    sql += "(ACCOUNT_ID, ORDER_ID, SHARES, PRICE, TIME) VALUES (" +
...
/*   match order   */
int match_order (connection* C, std::string& account_id,
                 std::string& order_id, std::string& sym,
  ...
  try {
    ...
    if (amount_ld < 0) {
      ...
      for (res = R.begin(); res != R.end(); ++res) {
        ...
        if (seller_amount_ld >= buyer_amount_ld) {
          matched_amount_ld = buyer_amount_ld;
          seller_amount_ld -= buyer_amount_ld;
          if (update_record(C, SELL, sym, order_id, seller_account_id,
                            buyer_account_id, matched_amount_ld,
                            matched_limit_ld) < 0) {
            return -1;
          }
        }
          ...
          if (update_record(C, BUY, sym, order_id, seller_account_id,
                            buyer_account_id, matched_amount_ld,
                            matched_limit_ld) < 0) {
            return -1;
          }
      ...
      /*   TODO: update amount in case there's un-finished order   */
    }
    ...
    /*   purchase goods, find sellers   */
      ...
      R = W.exec(sql);
      res = R.begin();
      long double curr_balance_ld = res1[0].as<long double>();
      
      if (curr_balance_ld < amount_ld * limit_ld) {
...
void place_order (connection* C, std::string& account_id,
  ...
  try {
    ...
    // check if client's sym share is enough
    sql = "SELECT \"" + sym + "\" FROM ACCOUNT WHERE ACCOUNT_ID = ";
    sql += W1.quote(account_id) + ";";
    R = W1.exec(sql);
    res = R.begin();
    if ((res[0].as<long double>()+amount_ld) < 0) {
      // insufficient shares
      response += "  <error sym=\"" + sym + "\" amount=\"" + amount +
                  "\" limit=\"" + limit + "\">\n" +
    ...
    /*   TODO   */
#if 0
    int stat = match_order(C, account_id, order_id, sym, amount, limit);
    if (stat == -1) {
      response += "  <error sym=\"" + sym + "\" amount=\"" + amount +
                  "\" limit=\"" + limit + "\">\n" +
    ...
    sql += W2.quote(account_id) + ", ";
    sql += W2.quote(std::to_string(order_id)) + ", ";
    sql += W2.quote(sym) + ", ";
    sql += W2.quote(amount) + ", ";
    sql += W2.quote(limit) + ");";
    W2.exec(sql);
