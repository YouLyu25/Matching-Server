...
int update_record (connection* C, work& W, int status, std::string& sym,
  ...
  std::cout << "update_record: " << sym << std::endl;
  try {
    std::string sql;
    result R;
    ...
    if (status == SELL) {
      ...
      sql = "SELECT ORDER_ID, AMOUNT FROM OPENED_ORDER WHERE ACCOUNT_ID = " +
            ...
            " AND PRICE = " + W.quote(std::to_string(matched_limit_ld)) + ";";
      R = W.exec(sql);
      res = R.begin();
...
int match_order (connection* C, work& W, std::string& return_order_id,
                 ...
                 std::string& response) {
  std::cout << "match_order: " << sym << std::endl;
...
void place_order (connection* C, std::string& account_id,
  ...
  std::cout << "place_order: " << sym << std::endl;
  try {
    std::string sql;
    work W(*C);
    result R;
...
void cancel_order (connection* C, std::string& sym, std::string& account_id,
                  std::string& order_id, std::string& response) {
  std::cout << "cancel_order: " << sym << std::endl;
  try {
    std::string sql;
    work W(*C);
    result R;
...
int handle_transactions (xmlpp::TextReader& reader, std::string& response) {
  std::cout << "handle_transactions: " << std::endl;
  try {
    ...
    do {
      ...
      /*   cancel order   */
        ...
        if (reader.get_attribute_count() == 1) { // if node has 1 attribute
          ...
          // cancel order
          cancel_order(C.get(), sym, account_id, order_id, response);
