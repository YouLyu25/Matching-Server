...
int update_record (connection* C, int status, std::string& sym,
    ...
    if (amount_ld < 0) {
      ...
      for (res = R.begin(); res != R.end(); ++res) {
        ...
        if (seller_amount_ld > matched_amount_ld) {
          ...
          if (update_record(C, SELL, sym, seller_account_id, buyer_account_id,
      ...
      /*   TODO: update amount in case there is un-finished order   */
    ...
    /*   purchase goods, find sellers   */
      ...
      for (res = R.begin(); res != R.end(); ++res) {
        ...
        if (buyer_amount_ld > matched_amount_ld) {
          ...
          if (update_record(C, BUY, sym, seller_account_id, buyer_account_id,
                            matched_amount_ld, matched_limit_ld) < 0) {
            return -1;
          }
        }
...
void place_order (connection* C, std::string& account_id,
  ...
  try {
    ...
    R = W.exec(sql);
    res = R.begin();
    if ((res[0].as<long double>() + amount_ld * limit_ld) < 0) {
    ...
    int stat = match_order(C, account_id, sym, amount, limit, response);
    if (stat == -1) {
      response += "  <error sym=\"" + sym + "\" amount=\"" + amount +
                  "\" limit=\"" + limit + "\">\n" +
